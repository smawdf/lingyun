...
- **设置窗口界面材质三档**（`ui_material`）：`acrylic` 亚克力（模糊 36px + 色调 ~48%）、
  `glass` 液态玻璃（模糊 30px + 色调 ~37%，最透）、`classic` 原生 Windows（不抓屏、纯色 + 方角 + 系统控件）。
  实现要点：设置窗口走 **AllowsTransparency（逐像素 alpha 的分层窗，和岛同一套机制）**，
  背景模糊**自己抓自己糊**（抓窗口背后那块屏幕 → WPF BlurEffect），不依赖系统亚克力——
  普通窗口里 `Transparent` 像素对 DWM 就是不透明黑，圆角外会直接变黑（"黑框"的根因），
  而系统亚克力在无边框窗口上边缘 artifact 多、还和分层窗互斥。抓屏前用
  `WDA_EXCLUDEFROMCAPTURE` 临时把自己排除、抓完立刻恢复（长期开着会让窗口在用户截图里消失）。
  材质明暗按实测背景亮度自动切浅色/深色（薄材质压深色桌面会"深底深字"，与岛上自适应同一条结论）
