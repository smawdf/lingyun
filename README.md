...
- 展开面板 6 页：计划 / 性能 / 天气 / **日程** / **日历** / **快捷**（点页签或滚轮切换；
  每页专属强调色 + 图标页签 + 顶部氛围光）
- **天气页**：当前卡右上角是**手绘矢量天气图标**（晴/多云/雨/雪/雾/雷，按天气码切换）并带分层动效——
  太阳光线 20 秒转一圈、云上下浮 3%、雨滴 4 滴错相下落、雪花边落边摆、闪电按不规则包络闪；
  动画写成**时间的纯函数**（不是有状态粒子，所以不漂移、与帧率无关），路径在单位坐标里建一次、
  每帧只做 canvas 变换；系统关掉动画时出静态姿态。三张小卡（湿度/风速/云量）来自 Open-Meteo 的 `current` 块
  （**请求里不能带 `current_weather=true`**，否则该块整块返回 null——踩过）
- **页面可直接编辑**：计划页点开关/动作分段/星期方块，点时间卡弹时间编辑窗；
  日历页点日期加入/移出计划（‹ › 翻月）；日程页点勾选圈完成/取消
- 旁挂圆点：媒体与计划同时存在时切换焦点
- **组合模式**（1.5.0 移植，默认关闭）：胶囊里同时显示**时间（带日期行）+ 硬件占用 + 实时网速（可选）+ 媒体**，
  宽度按内容自动伸缩（`composite` / `composite_clock` / `composite_hardware` / `composite_media`）；
  时间槽按 `88:88` 定宽、硬件百分比按 `100%` 定宽、网速按 `999.9 MB/s` 定宽——倒计时和数字跳动时**宽度不抖**；
  宽度夹在 220–900，内容不足 220 时（例如关掉时间只留硬件 + 网速）模块整体居中而不是贴左
...
「快捷」页底部还有一条**音频区**：主音量（点或拖）+ 静音钮 + 「输出 / 输入」两行当前设备；
点任一行**就地展开设备列表**（点一项即切默认设备、列表内滚轮翻页、点别处收起）。
设备切换走 Windows 未公开的 `IPolicyConfig`（微软没有公开 API），**切完会回读校验**——
被音频增强软件（如 FxSound）抢回去时如实提示失败，而不是假装成功。
...
- **三种媒体页样式**（「岛设置 → 外观 → 媒体页」实时切换，写进 `media_style`）：
  ...
  **进度条两端同一行显示已播时间与总时长**（时间紧贴条的两端），进度条**加粗到 6px**、**没有拖动圆点**；
  **进度条可以按住拖动定位**（拖动中本地预览位置，松手才提交——暂停时很多播放器不更新
  SMTC 位置，只发 seek 界面会弹回原位，看着像"拖不动"）；
  **沉浸（b）也改用封面取色**（背景是封面糊出来的，进度条/频谱/歌词再用调色板蓝会打架）；
  A 精修保留调色板强调色（否则 A 和 C 观感几乎一样）
...
- **音量与音频设备**：「快捷」页底部常驻一条音频区——主音量（点/拖都行）+ 静音钮，
  下面「输出 / 输入」两行显示当前设备，点一下就地展开设备列表（点一项即切默认设备，列表内滚轮翻页）。
  没在放歌时也能调；设备切换走 Windows 未公开的 `IPolicyConfig`（微软无公开 API），
  **切完会回读校验**，被音频增强软件（如 FxSound）抢回去时如实提示而不是假装成功。
  媒体页右下角另有喇叭图标，**点它弹出竖向调节条**（更符合直觉，不再常驻横条）——
  竖槽按住可拖、点哪到哪，弹出条顶部是静音开关，点别处收起弹出条（不会顺手把面板也收掉）；
  紧凑媒体态下**滚轮直接调音量**（±2%，展开态滚轮仍留给翻页）；无播放设备时整个图标隐藏
...
- 单实例互斥 + 命名管道唤出
- 托盘：显示 / 暂停计划 / **岛设置…** / 切换显示器 / 音频频谱 / 显示歌词 /
  **消息通知** / 开机自启 / 退出（开关勾选即生效并写回配置）；
  「浅色主题」已按用户要求移除——深浅在「岛设置 → 外观 → 主题」里选，托盘那个是重复入口
- 展开面板**没有关闭按钮**：收起靠点空白处（含页签条空白、以及**岛外的桌面空白**——靠一个只观察不拦截的全局鼠标钩子，不需要管理员权限）或**展开自动关闭**（「位置与大小」里可调 0–10 秒，0 = 永不自动关闭）
...
- 「关于」页只放用户该看到的：版本、配置目录（一键打开）、开机自启、源代码仓库、许可与第三方声明；
  命令行诊断入口属于开发向内容，已从界面移除
- 标题栏图标用应用自己的图标（`Assets/lingyun.ico`，同时设为 exe 图标）
- 设置窗口**可以拉伸**（拖边缘/四角改尺寸，最小 620×420；无边框窗口靠 WM_NCHITTEST 命中边缘；
  拉伸过程中不重新落位，只有首次显示时才落到岛下方）；
- 设置窗口**不再强制置顶**（普通窗口层级，被别的窗口盖住是正常的）：岛在编辑窗打开期间会自己让出置顶，
  所以两者不会互相压住；
  ✕ 是不涂底色的"裸按钮"（只有字形，悬停轻微压暗）；亚克力档形状由窗口区域裁出、不再画自绘包边
  （避免和锯齿区域边叠成"两层"），玻璃档才用包边定义轮廓；
  切换材质时会**显式清掉上一次的窗口裁剪区域**（否则从亚克力切到玻璃后，系统里那个 10px 锯齿区域还留着，
  和自绘的 20px 圆角错位，四角会露出方角块）；
  设置窗口的按钮都走同一套扁平模板（含 ✕ 与「完成」）：没有系统默认的悬停底色、没有焦点虚线框；
  滑杆**点哪到哪**（不是按固定步长加减）；岛的液态玻璃自适应换色加了中值平滑 + 连续两次一致的驻留
- **外观是一个整体**（岛与设置窗口同一套，不再分成两个设置）：**亚克力**（默认）或 **液态玻璃**；
  亚克力档下再选深浅（深色 / 浅色 / 跟随系统，记在 `base_theme` 里，从玻璃切回来会恢复）。
  设置窗口材质由主题推导：液态玻璃 → 与岛同款半透明（清晰透明、不做模糊，**跟随背景透明度**）；
  其余 → 系统合成器模糊（DWM，桌面被糊在面板后面、移动跟手）。
  亚克力只能走 `SetWindowCompositionAttribute` 的 accent 私有属性：**Win11 官方背景材质实测用不了**——
  `DWMWA_SYSTEMBACKDROP_TYPE`（桌面亚克力）+ `DwmExtendFrameIntoClientArea(-1)` + `DWMWA_REDIRECTIONBITMAP_ALPHA`
  三个属性在 26100 上全部返回 S_OK，但窗口内亮度恒为 **0.000（纯黑）**：WPF 不给普通窗口的重定向位图写 alpha，
  透明像素被 DWM 当成不透明黑，材质被自己的黑底盖住（官方材质还要求窗口不是分层窗，两者正好互斥）。
  `lingyun.exe --acrylic-probe` 会把这条实测结论连带"材质到底活没活"一起验出来。
  色调只画一次（亚克力交给 DWM，玻璃由我们自己画）；圆角只有亚克力需要裁窗口区域，
  玻璃的圆角完全由我们画（避免多出一道锯齿弧）。
...
| `theme` | `dark`（纯黑 `#000000`）/ `light`（暖白面板）/ `system`（跟随系统）/ `liquid-glass`（液态玻璃） |
| `opacity` | 背景不透明度 40–100%（默认 100）；只压背景与材质，文字/强调色保持不透明；液态玻璃同样跟随 |
| `window_opacity` | **设置窗口自己的**背景不透明度 40–100，`-1`（默认）= 跟随岛；两者最优值不同（岛小、字直接压在玻璃上；窗口大、有卡片和留白托着），而且岛的透明度还参与自适应换色判据，所以可以分开调 |
...
| `auto_collapse_ms` | 岛展开面板的自动关闭时长（毫秒，0–10000，默认 900；0 = 永不自动关闭） |
| `topmost_mode` | 置顶层级：`always` 始终置顶（默认）/ `normal` 普通层 / `auto` 自动（前台全屏时让位，退出全屏自动恢复） |
| `base_theme` | 亚克力档下的深浅（`dark`/`light`/`system`，默认 `dark`）；`theme=liquid-glass` 时记住它，切回来恢复 |
| `collapse_on_blank` | 点展开面板空白处（含页签条空白）是否收起，默认 `true` |
| `toast` | 系统通知弹窗，默认 `true`；关掉后岛不再接管胶囊（托盘「消息通知」同源） |
| `composite` | 组合模式总开关（默认 `false`）；开启后胶囊同屏显示多个模块 |
| `composite_clock` / `composite_hardware` / `composite_network` / `composite_media` | 组合模式模块：时间（带日期行）/ CPU+内存 / **实时网速 ↓↑** / 媒体（前三者默认开、网速默认关；全关时强制保留时间） |
...
| 现象 | 处理 |
|------|------|
| 双击无反应 / 秒退 | 任务管理器结束所有 `lingyun.exe`（进程名显示为「灵云」）后再启动（单实例） |
