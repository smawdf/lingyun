...
- **设置窗口界面材质三档**（`ui_material`）：`acrylic` 亚克力（系统真模糊 + 65% 色调）、
  `glass` 液态玻璃（同一套模糊 + 更薄的玻璃色调 ~55% + 大圆角；WPF 做不了边缘折射，界面上会如实标注"这一档是近似"）、
  `classic` 原生 Windows（纯色 + 方角 + 系统控件）。模糊统一走 Win10 1803+ 的合成属性接口，
  老系统退回纯色不假装；圆角由窗口区域自己裁——DWM 自带圆角只有 ~8px，和我们要的大圆角对不齐会露馅（四角"分层黑"）
