...
- **音量**：展开媒体页右下角只留一个喇叭图标，**点它弹出竖向调节条**（更符合直觉，不再常驻横条）——
...
- 注意：岛体是 `UpdateLayeredWindow` 分层窗口，拿不到桌面像素，所以岛上的液态玻璃是**应用内材质**，
  不是 Win11 的桌面级 Acrylic；自适应是“抓屏 → 算亮度 → 换材质”，岛本身不做实时桌面折射。
  **设置窗口的液态玻璃档是"一整块"玻璃**：面板背景由**一张图**铺满（不是"半透明色调层 + 再叠一圈环带"，
  那会被眼睛看成两层——实测旧做法在离边界 1~12px 处是一条近乎不透明的深色带，最外 1px 又只有 18% 不透明度，
  放大就是两个同心圆角矩形）。这张图里：本体 = 材质色 + 材质 alpha；圆角内侧约 12px 环带 = 把窗外背景
  沿法线外推 3→26px 采样（把更宽的一条背景**压进**窄环带，那才是玻璃边缘的弯折）后，按**同一个透过率**
  混进材质色，R/G/B 再错开 ±2.4px 产生轻微色散。因此纯色壁纸下边缘与本体在数学上完全一致（自测断言差异 ≤1.5），
  只有背景有结构时才看得到弯折。不做整窗模糊；抓屏失败时退回透明色调层；透明度 40–100% 同步作用于整块。
  `lingyun.exe --edge-probe` 在真实窗口上验这块面板与折射，`--settings-render` 把真实视觉树渲染出来并逐像素量
  alpha/颜色剖面（一层材质应当是平的，出现台阶就是两层）。
...
- **外观是一个整体**（岛与设置窗口同一套，不再分成两个设置）：**亚克力**（默认）或 **液态玻璃**；
  ...
  设置窗口材质由主题推导：液态玻璃 → 与岛同款半透明（清晰透明、**圆角边缘做屏幕采样折射与轻微 RGB 色散**，中心保持透明，跟随背景透明度）；
  ...
  对"会移动的窗口"这条是死路，已废弃。现在 glass 档只刷新圆角边缘窄环带，采样失败时保留透明色调，
  不把整窗实时模糊；DWM 的亚克力仍不做边缘折射。
