...
- 注意：岛体是 `UpdateLayeredWindow` 分层窗口，拿不到桌面像素，所以岛上的液态玻璃是**应用内材质**，
  ...
  **设置窗口的液态玻璃档另有真实的边缘折射**：每 100ms 抓取窗口外扩 48px 的一小块桌面，只在圆角内侧约 12px 环带取样，
  采样点沿边缘法线从 3px 外推到 26px —— 把窗外一条更宽的背景**压进**窄环带，那才是玻璃边缘的"弯折"观感
  （早期版本位移近似恒定，看着仍然像一层半透明）；R/G/B 三通道再错开 ±2.4px 产生轻微色散，
  并沿法线做一次对比度增强，壁纸/图标等有结构的背景弯折更明显。
  另压一道**相对面板色调**的棱边（浅面板压暗、深面板提亮）：窗外是纯色/浅色壁纸时，
  "更透的环带"和近白面板几乎同色，数学上就看不见折射，棱边保证任何壁纸上都有可见的玻璃边界。
  中心区域完全透明、不覆盖控件，不做整窗模糊；抓屏失败时回退到原来的透明色调，透明度 40–100% 同步作用于折射层。
  `lingyun.exe --edge-probe` 会在真实窗口上验证这一切（环带非空/颜色多样/中心全透明/圆角命中/明暗差可辨/单次耗时）。
