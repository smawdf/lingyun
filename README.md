...
- **岛设置窗口**：主题（深色 / 浅色 / **跟随系统** / **液态玻璃**）、背景透明度（40–100% 滑杆 +
  轻透/半透/不透明三档预设）、
  胶囊与展开大小、水平位置与距顶部、组合模式开关与模块、性能页网速（实时下载/上传）、系统通知、闲置自动隐藏、
  歌词（显示 / 卡拉OK / 延迟补偿）、本地版本号；滑杆拖动实时预览，关闭时写盘
...
| `theme` | `dark`（纯黑 `#000000`）/ `light`（暖白面板）/ `system`（跟随系统）/ `liquid-glass`（液态玻璃，恒浅色）；托盘「浅色主题」可直接切深浅 |
