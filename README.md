...
- **三种媒体页样式**（「岛设置 → 外观 → 媒体页」实时切换，写进 `media_style`）：
  `a` **精修**（默认：大封面、大字号标题、歌词带）、
  `b` **沉浸**（封面模糊铺满整底 + 底部玻璃控制条）、
  `c` **氛围海报**（封面取色双光斑 + 微倾大封面 + 大字号标题）；
  三者的传输键与进度条统一为参考形态：**上一曲 / 播放暂停 / 下一曲是纯矢量字形（无圆底）**，
  **进度条两端同一行显示已播时间与总时长**，进度条上**没有拖动圆点**；
  B/C 的模糊与取色都在换曲时重算一次并缓存，逐帧成本≈0
- 展开面板**自动回缩可调**（`auto_collapse_ms`，默认 900ms，0 = 永不自动收起）：
  在「位置与大小」里有滑杆；回缩前会二次确认光标确实不在岛上（分层窗偶发 WM_MOUSELEAVE
  时不会误收），点传输键/进度条也不会再收起面板
...
- **岛设置（主页式，820×580）**：左侧分区导航 + 右侧内容（内容区右侧是细的浮层滚动条：轨道透明、
  半透明圆角滑块、无上下箭头，和面板材质一致），五个分区 ——
...
- **设置窗口界面材质三档**（`ui_material`，只影响设置窗口，岛的材质在 `theme` 里）：
  `acrylic` 亚克力（**系统合成器模糊**，桌面被糊在面板后面）、
  `glass` 液态玻璃（**与岛同一套材质**：清晰透明、不做模糊，背后内容直接透出来）、
  `classic` 原生 Windows（纯色 + 方角 + 系统控件）。
  实现要点：窗口 `AllowsTransparency=true`（分层窗，四角真透明、无黑框）+ `WindowStyle=None`
  + 近透明背景（alpha=1，全 0 时部分系统不参与合成），**背景模糊交给 DWM 合成器**
  （`SetWindowCompositionAttribute` 的亚克力）——它是合成器的一部分，窗口移动时模糊跟手、零延迟。
  参考实现：[riverar/sample-win32-acrylicblur](https://github.com/riverar/sample-win32-acrylicblur)（WPF 亚克力事实标准样例）。
  曾经试过"自己抓屏 + WPF BlurEffect"：静态观感能对，但窗口一移动就是卡顿式的（抓屏 1Hz + CPU 模糊），
  对"会移动的窗口"这条是死路，已废弃。DWM 的亚克力不做边缘折射，所以 glass 档是"模糊 + 淡色调 + 大圆角"的近似，
  真折射只在岛那边（Skia 自绘）
- 设置窗口**整窗可拖**（除控件外任意空白处），不是只有标题条那一条
...
| `auto_collapse_ms` | 展开面板自动回缩时长（毫秒，0–10000，默认 900；0 = 永不自动收起） |
