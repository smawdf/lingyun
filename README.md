...
- **岛设置窗口**：主题（深色 / 浅色 / **跟随系统**）、背景不透明度（40–100%）、
...
| `theme` | `dark` / `light` / `system`（跟随系统浅色/深色，切换后约 2 秒内生效） |
...
| `theme` | `dark`（纯黑 `#000000`）/ `light`（暖白面板）；托盘「浅色主题」可直接切 |
| `opacity` | 背景不透明度 40–100%（默认 100）；只压背景，文字/强调色保持不透明 |
...
| `quick_custom` | 快捷页自定义程序（最多 3 个；`[{name, path, color}]`，单击启动） |
